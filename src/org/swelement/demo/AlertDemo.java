...
public class AlertDemo {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            btnPanel.setBorder(BorderFactory.createTitledBorder("点击按钮弹出 AstAlert（观察淡入动画）"));
            ...
            scroll.setBorder(BorderFactory.createTitledBorder("AstAlert 展示区（观察关闭时收缩动画）"));
            ...
            // 4 个类型弹出按钮（含描述+可关闭）
            AstButton btnSuccess = new AstButton("弹出 Success", AstButton.SUCCESS, false);
            btnSuccess.addActionListener(e -> {
                AstAlert a = new AstAlert(AstAlert.SUCCESS, "操作成功",
            ...
            btnWarning.addActionListener(e -> {
                AstAlert a = new AstAlert(AstAlert.WARNING, "存储空间不足",
            ...
            btnInfo.addActionListener(e -> {
                AstAlert a = new AstAlert(AstAlert.INFO, "温馨提示",
            ...
            btnError.addActionListener(e -> {
                AstAlert a = new AstAlert(AstAlert.ERROR, "网络请求失败",
            ...
            btnSimple.addActionListener(e -> {
                AstAlert a = new AstAlert(AstAlert.INFO, "这是一条精简消息，无描述、不可关闭", null, false);
                addAlert.accept(a);
            });
            ...
            btnClear.addActionListener(e -> {
                for (Component c : alertArea.getComponents()) {
                    if (c instanceof AstAlert) {
                        AstAlert alert = (AstAlert) c;
                        alert.close(() -> {});
                    }
                }
            ...
            alertArea.add(new AstAlert(AstAlert.SUCCESS, "成功提示（静态展示）",
                    "这是一条成功提示信息，带描述且可关闭。点击右上角 × 查看收缩动画。", true));
            alertArea.add(Box.createVerticalStrut(8));
            alertArea.add(new AstAlert(AstAlert.WARNING, "警告提示（静态展示）",
                    "这是一条警告提示信息，带描述且可关闭。", true));
            alertArea.add(Box.createVerticalStrut(8));
            alertArea.add(new AstAlert(AstAlert.INFO, "消息提示（精简版）", null, false));
            alertArea.add(Box.createVerticalStrut(8));
            alertArea.add(new AstAlert(AstAlert.ERROR, "错误提示（静态展示）",
