...
public class Badge extends JComponent {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int textW = !dot ? fm.stringWidth(text) : 0;
        // 根据内容动态计算角标宽度和圆心
        int badgeW = dot ? 10 : (count > 99 ? Math.max(24, textW + 10) : 18);
        int cx = getWidth() - badgeW / 2 - 2;
        int cy = badgeW / 2 + 2;
        ...
        if (dot || count <= 99) {
            ...
            g2.fillRoundRect(cx - badgeW / 2, cy - badgeW / 2, badgeW, badgeW, 8, 8);
        }
        if (!dot) {
